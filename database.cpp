...
static const char *create_database = 
...
void database_init()
{
    ...
    if (NULL == mysql_real_connect(&mydata_A, "localhost", "root", "Yy649535675", "sys", 3306, NULL, CLIENT_MULTI_STATEMENTS))
    {
        printf("connect failed %u: %s\n\n", mysql_errno(&mydata_A), mysql_error(&mydata_A));
        perror("");
        exit(1);
    }
    MYSQL_RES *pRes;
...
void save_board_data(string json_data)
{
    ...
    string temp = j["temp"];
    string humi = j["humi"];
    string light = j["light"];
    string smoke = j["smoke"];
    ...
    insert = insert + "\'0\',\'" + year + "\',\'" + month + "\',\'" + date + "\',\'" + wday + "\',\'" +
             ttime + "\',\"" + name + "\",\"" + location + "\",\'" + temp + "\',\'" + humi + "\'," + "\'" + light + "\'," + "\'" + smoke + "\',"+"\'" + state +"\');";
    // cout<<insert<<endl;
